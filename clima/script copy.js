...
function showInfo(json) {                                                                                                       //Função para exibir o resultado da busca na tela
    ...
    document.querySelector('.title').innerHTML = `${json.name},${json.country}`;                                                //Exibe o nome da cidade e o país
    document.querySelector('.tempInfo').innerHTML = `${json.temp} <sup>ºC</sup>`;                                               //Exibe a temperatura
    document.querySelector('.windInfo').innerHTML = `${json.windSpeed} <span>km/h</span>`;                                      //Exibe a velocidade do vento

    document.querySelector('.temp img').setAttribute('src', `http://openweathermap.org/img/wn/${json.tempIcon}@2x.png`);        //Exibe o icone do estado do tempo
    document.querySelector('.windDot').style.transform = `rotate(${json.windAngle-90}deg)`;                                     //Exibe o ângulo do vento

    document.querySelector('.result').style.display = 'block';                                                                  //Após carregar as informações acima, quando houver algum resultado, mostra a area de exibição de resultados.
